...
public class AIAcolyteOfFrost : ActorAI
{
    [SerializeField]
    ...
    protected override IEnumerator UseAbilityRoutine(AbilityState selectedAbility)
    {
        ...
        if (selectedAbility.CurrentAbility.name == "Icicle Acolyte" || selectedAbility.CurrentAbility.name == "Dash Acolyte")
        {
            MoveToTarget();
        ...
        else if (selectedAbility.CurrentAbility.name == "Dash")
        {
            if(rhitRight)
            {
    ...
    protected override IEnumerator AIBehaviourRoutine()
    {
        ...
        while (true)
        {
            ...
            while (SelectedAbility == null)
            {
                ...
                {
                    SelectedAbility = Act.State.Abilities.Find(x => x.CurrentAbility.name == "Dash" && x.CurrentCD <= 0f);
                    
                    // if(SelectedAbility == null)
                    // {
                    //     SelectedAbility = Act.State.Abilities.Find(x => x.CurrentAbility.name == "Bless Acolyte" && x.CurrentCD <= 0f);
                    // }
                    ...
                    if(SelectedAbility == null)
                    {
                        SelectedAbility = Act.State.Abilities.Find(x => x.CurrentAbility.name == "Icicle Acolyte" && x.CurrentCD <= 0f);
                    }
                    ...
                    if(SelectedAbility == null)
                    {
                        ...
                        {   
                            IciclesShot = 0;
                            SelectedAbility = Act.State.Abilities.Find(x => x.CurrentAbility.name == "Dash Acolyte" && x.CurrentCD <= 0f);
                        }
                    }
