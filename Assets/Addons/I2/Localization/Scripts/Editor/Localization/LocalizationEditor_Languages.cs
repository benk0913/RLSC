...
namespace I2.Loc
{
	public partial class LocalizationEditor
	{
        ...
        {
            TranslateAllToLanguage(language, () =>
            {
                int currentIndex = mLanguageSource.GetLanguages().IndexOf(language);
                currentIndex++;
                if (currentIndex < mLanguageSource.GetLanguages().Count)
                {
                    IterateAutoTranslateAll(mLanguageSource.GetLanguages()[currentIndex]);
                }
            });
        }
        ...
        public void TranslateAllToLanguage (string lanName, Action onComplete = null)
		{
			...
			GoogleTranslation.Translate (mTranslationRequests, (Dictionary<string,TranslationQuery> reqs, string error) => 
            {
                OnLanguageTranslated(reqs, error);
                onComplete?.Invoke();
            });
		}
